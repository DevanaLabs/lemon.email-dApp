          ...
          mail.map( (mail, i) =>
            <div className="single-mail" key={i}>
              <h3 className="single-mail-from">
                ...
                <div className="single-mail-meta-wrapper">
                  <h3 className="single-mail-meta">IPFS Hash: <a target="_blank" href={"https://ipfs.io/ipfs/" + (mail.ipfsHash ? mail.ipfsHash : this.props.location.pathname.split('/').pop())}>{mail.ipfsHash ? mail.ipfsHash : this.props.location.pathname.split('/').pop()}</a></h3>
                  {<h3 className="single-mail-meta">Transaction Hash: <a target="_blank" href={"https://kovan.etherscan.io/tx/" + (mail.transactionHash ? mail.transactionHash : this.props.singleMail.curentMailTransactionHash)}>{mail.transactionHash ? mail.transactionHash : this.props.singleMail.curentMailTransactionHash}</a></h3>}
                </div>
              ...
              <div className="attachments-list">
                {
                  mail.attachments.length > 0 && this.props.urlLocation !== "sent" &&
                  mail.attachments.map(function(attachment, index) {
                    return <a
                      className="single-mail-attachment icon-download"
                      onClick={this.props.downloadAttachment.bind(null, attachment)}
                      key={index}>
...
SingleMail.propTypes = {
  ...
  downloadAttachment: React.PropTypes.func.isRequired
};
